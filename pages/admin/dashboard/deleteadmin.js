...
export default class PostList extends React.Component {
  ...
  componentDidMount() {
    axios.get(`https://house-rent-management-system-production.up.railway.app/admin/adminindex`)
      .then(res => {
  ...
  deleteRow(id, e) {
    axios.delete(`https://house-rent-management-system-production.up.railway.app/admin/deleteadmin/${id}`)
      .then(res => {
        console.log(res);
  ...
  render() {
    return (<>
      ...
      <div className="p-24 flex justify-center items-center" align="center" style={{ background: 'linear-gradient(204deg, rgba(204,255,229,1) 0%, rgba(204,255,229,1) 100%)' }}>
        <h1 className="text-blue-600 text-3xl font-bold my-4 border-b-2 border-blue-400">Delete Admin</h1>
        ...
        <table className="table-fixed w-4/5 bg-white rounded-md shadow-lg">
          <thead className="bg-blue-400 text-white">
            <tr>
              ...
              <th className="w-2/12 p-2 border border-blue-400">Date of Birth</th>
          ...
          <tbody>
            {this.state.posts.map((post) => (
              <tr key={post.id}>
                ...
                <td className="p-2 border border-blue-400">{post.uname}</td>
                <td className="p-2 border border-blue-400">{post.dob}</td>
                <td className="p-2 border border-blue-400">{post.address}</td>
                <td className="p-2 border border-blue-400">{post.pass}</td>
